...
class MotionToast extends StatefulWidget {
  ...
  MotionToast({
    ...
    this.closeAnimationType = AnimationType.fromBottom,
    this.closeAnimationDuration = const Duration(
      milliseconds: 800,
    ),
  ...
  late AnimationType closeAnimationType;
    ...
    switch (widget.closeAnimationType) {
      case AnimationType.toLeft:
        if (widget.position == MotionToastPosition.top) {
          debugPrint(
            'MotionToastPosition Top ${MediaQuery.of(context).size.width}',
          );
          offsetAnimation = Tween<Offset>(
            begin: const Offset(0, 0.3),
            end: const Offset(-2, 0.3),
          ).animate(curveAnimation)
            ..addListener(() {
              setState(() {});
            });
        } else if (widget.position == MotionToastPosition.center) {
          if (widget.animationType == AnimationType.fromRight) {
            offsetAnimation = Tween<Offset>(
              begin: const Offset(0, -0.3),
              end: const Offset(2, -0.3),
            ).animate(curveAnimation)
            ...
            offsetAnimation = Tween<Offset>(
              // begin: const Offset(0, 0.3),
              // end: Offset(MediaQuery.of(context).size.width, 0.3),
              begin: const Offset(0, 0.3),
              end: const Offset(2, 0.3),
            ).animate(curveAnimation)
              ..addListener(() {
      ...
      case AnimationType.toRight:
        if (widget.position == MotionToastPosition.top) {
          ...
          if (widget.animationType == AnimationType.fromLeft) {
            offsetAnimation = Tween<Offset>(
              begin: const Offset(0, -0.3),
              end: const Offset(-2, -0.3),
            ).animate(curveAnimation)
            ...
            offsetAnimation = Tween<Offset>(
              ...
              ..addListener(() {
                debugPrint('Hello offsetAnimation${offsetAnimation.value}');
  ...
  void _initializeAnimation() {
    ...
    switch (widget.animationType) {
      ...
      case AnimationType.fromTop:
        if (widget.position == MotionToastPosition.center) {
          offsetAnimation = Tween<Offset>(
            begin: const Offset(-0.3, 0.3),
            end: const Offset(0, 0.3),
          ).animate(curveAnimation);
  ...
  Widget _buildMotionToast() {
    return MotionToastBackground(
      ...
      child: MotionToastContent(
        ...
        title: widget.title,
        svgIconAsset: widget.svgIconAsset,
